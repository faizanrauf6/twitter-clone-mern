export default function validate(values, purpose) {
  let errors = {};

  const { name, username, email, password, confirmPassword } = values;
  const emailRegex =
    /(([^<>()\[\]\\.,;:\s@"]+(\.[^<>()\[\]\\.,;:\s@"]+)*)|(".+"))@[*[a-zA-Z0-9-]+.[a-zA-Z0-9-.]+]*/; //eslint-disable-line
  ...
  if (purpose === 'signup') {
    ...
    if (!password) {
      ...
      !/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[a-zA-Z\d@$.!%*#?&]/.test(password)
    ) {
      errors.erPassword =
        'Password must contain at least 8 characters, a number, an uppercase, a lowercase, and a special character';
    }
